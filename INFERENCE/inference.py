...
with torch.no_grad():
    for t in reversed(range(1,T)):
        ...
        # Loss
        loss = loss_criterion(x0, x0_hat.squeeze(1))
...
if LOCAL:
    ...
    fig.savefig(str(path  / f'{PATIENT}_{PLANE}_{SLICE_IDX}_{delta[0]}_full_chain.png'))
    print(bcolors.OKGREEN + f'Succesfully saved result to {str(path  / f"{PATIENT}_{PLANE}_{SLICE_IDX}_{delta[0]}_full_chain.png")}' + bcolors.ENDC, flush=True)
    plt.close()
